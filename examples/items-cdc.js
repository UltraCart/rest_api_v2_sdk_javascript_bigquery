...
/**
 * Change-data-capture for ITEMS (catalog) — in one query, pull the current state of every item
 ...
 * `ultracart_dw_streaming`. Override via env vars:
 ...
 *
 * Run:
 *   node examples/items-cdc.js ACME 2026-06-22T00:00:00Z
 */
...
async function main() {
  ...
  if (!since) {
    console.error('Usage: node examples/items-cdc.js <MERCHANT_ID> <since ISO8601>');
    process.exit(1);
  }
  ...
  const sql = `SELECT * FROM ${VIEW}.uc_items
                WHERE merchant_id = @mid
                  AND merchant_item_oid IN (
                        SELECT merchant_item_oid FROM ${STREAMING}.uc_item_streaming
  ...
  console.log(`Synced ${count} changed items for ${merchantId} since ${since}.`);

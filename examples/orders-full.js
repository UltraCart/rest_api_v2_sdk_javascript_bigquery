...
/**
 * Full backfill of ORDERS — stream the entire history once to seed your store.
 * Constant memory (async iterator). Make your write idempotent on order_id.
 *
 ...
 *   UC_PROJECT_ID=ultracart-dw-acme UC_VIEW_DATASET=ultracart_dw_medium node examples/orders-full.js ACME
 */
...
async function main() {
  ...
  const sql = `SELECT * FROM ${VIEW}.uc_orders WHERE merchant_id = @mid`;
  const params = { mid: merchantId };

  const est = await ucbq.dryRun(sql, { params });
  console.log(`Backfill orders from ${ucbq.projectId}.${VIEW}: ~${est.gigabytesProcessed.toFixed(2)} GB (~$${est.estimatedCostUsd.toFixed(2)}).`);
